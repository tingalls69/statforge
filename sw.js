// Ascendry public alpha cache manifest — Lorelei preview 0.5.1
importScripts('./js/version.js');

const VERSION = globalThis.SF_VERSION || '0.5.1';
const CACHE = `ascendry-alpha-v${VERSION}`;
const v = `?v=${encodeURIComponent(VERSION)}`;
const ASSETS = [
  './', './index.html', `./index.html${v}`,
  `./css/styles.css${v}`, `./css/alpha.css${v}`, `./css/alpha-avatar.css${v}`, `./css/alpha-game.css${v}`, `./css/alpha-public-polish.css${v}`, `./css/alpha-quest-compact.css${v}`, `./css/alpha-workout.css${v}`, `./css/alpha-workout-fantasy.css${v}`,
  `./js/version.js${v}`, `./js/data.js${v}`, `./js/storage.js${v}`, `./js/alpha-accent.js${v}`, `./js/combat.js${v}`, `./js/alpha-upgrade.js${v}`, `./js/alpha-life-quests.js${v}`, `./js/alpha.js${v}`, `./js/alpha-avatar.js${v}`,
