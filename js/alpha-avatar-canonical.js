(() => {
  ...
  const BODY = {
    Lean: { shoulder: 45, waist: 29, hip: 35 },
    Average: { shoulder: 51, waist: 35, hip: 41 },
    Broad: { shoulder: 61, waist: 42, hip: 46 },
    Stocky: { shoulder: 58, waist: 49, hip: 53 }
  };
  ...
  function identity() {
    ...
    const className = save.character?.class
      || save.gameCombat?.selectedClass
      || save.onboarding?.selectedGameClass
      || save.onboarding?.selectedClass
      || 'Fighter';
    const build = save.character?.build
      || save.character?.featurePackage
      || save.gameCombat?.selectedBuild
      || save.onboarding?.selectedBuild
      || save.onboarding?.selectedPackage
      || 'Vanguard';
  ...
  function gearSvg(className, build, secondary) {
    ...
    if (className === 'Wizard') {
      const crystal = normalized.includes('frost') ? '#8bd4e9' : normalized.includes('ember') ? '#df7a4a' : '#9a79ce';
      return `<g transform="translate(259 173) rotate(13)"><path d="M0 0v206" stroke="#6a4b31" stroke-width="10"/><path d="M0 5q-28-22-25-49M0 5q28-22 25-49" fill="none" stroke="#6a4b31" stroke-width="8"/><path d="M0-58l16 20-16 20-16-20z" fill="${crystal}" stroke="#e6eadf" stroke-width="3"/></g>`;
    }
    if (className === 'Ranger') {
      return `<g transform="translate(111 153) rotate(-17)"><path d="M0 0h31l-7 132H7z" fill="#694a34" stroke="#33271f" stroke-width="4"/><path d="M8 5l-8-45M17 5l2-48M26 5l13-42" stroke="#c3a66b" stroke-width="3"/></g><g transform="translate(260 165) rotate(16)"><path d="M0 0q-50 76 0 156" fill="none" stroke="#8a6039" stroke-width="8"/><path d="M0 0v156" stroke="#e0d3a7" stroke-width="2"/></g>`;
    }
    ...
    if (className === 'Rogue') {
      return `<g transform="translate(264 274) rotate(22)"><path d="M0 0h5l1 72-4 14-4-14z" fill="#bdc4bf" stroke="#303633" stroke-width="3"/><path d="M-9 7h22" stroke="#b28a49" stroke-width="5"/></g><path d="M111 239q-20 40-8 91h43l8-87z" fill="#64462f" stroke="#30231b" stroke-width="5"/>`;
    }
    if (normalized.includes('breaker')) {
      return `<g transform="translate(256 174) rotate(18)"><path d="M0 0v180" stroke="#654b36" stroke-width="10"/><path d="M-8 3q-39 4-48 40 32 10 57-11z" fill="#a6aeaa" stroke="#343b39" stroke-width="4"/></g>`;
    }
  ...
  function bodySvg(visual) {
    ...
    if (visual.presentation === 'Masculine') {
      shape.shoulder *= 1.04;
      shape.hip *= .97;
    } else if (visual.presentation === 'Feminine') {
      shape.shoulder *= .96;
      shape.hip *= 1.05;
    }
    ...
    let leftHandX = left - 7;
    let leftHandY = hipY + 7;
    let rightHandX = right + 7;
    let rightHandY = hipY + 7;
    if (visual.pose === 'Confident') {
      leftHandX = center - shape.waist - 4;
      leftHandY = waistY + 18;
    } else if (visual.pose === 'Relaxed') {
      rightHandX = center + 15;
      rightHandY = waistY + 27;
    }
    ...
    if (visual.outfit === 'Traveler') {
      outfit += `<path d="M${left - 2} ${top + 4}Q${center} ${top + 35} ${right + 2} ${top + 4}l-11 45q-40-24-78 0z" fill="${secondary}" stroke="${outline}" stroke-width="4"/><path d="M${center - 34} ${top + 31}L${center + 30} ${waistY + 66}" stroke="#76553a" stroke-width="8"/><path d="M${center + 34} ${waistY + 61}q30 8 26 50h-44q-5-35 18-50z" fill="#62452f" stroke="${outline}" stroke-width="4"/>`;
    } else if (visual.outfit === 'Apprentice') {
      outfit += `<path d="M${left + 10} ${top + 11}Q${center} ${top + 24} ${right - 10} ${top + 11}L${center + 20} ${hipY + 82}H${center - 20}Z" fill="${secondary}" stroke="${outline}" stroke-width="4"/><path d="M${center} ${top + 25}v${hipY - top + 54}" stroke="#c8aa5c" stroke-width="4"/>`;
    } else {
      outfit += `<path d="M${center - 18} ${top + 5}l18 25 18-25" fill="none" stroke="${secondary}" stroke-width="8"/><path d="M${center - 30} ${waistY + 10}l-7 73h32l5-70 5 70h32l-7-73z" fill="${secondary}" stroke="${outline}" stroke-width="4"/>`;
    }
  ...
  function renderLoreleiHead(root, visual) {
    try {
      ...
      img.alt = `${visual.name || 'Character'} face`;
      img.addEventListener('load', () => root.classList.add('head-ready'), { once: true });
      img.addEventListener('error', () => root.classList.remove('head-ready'), { once: true });
      img.src = loreleiHead(visual);
      root.append(img);
      if (img.complete && img.naturalWidth > 0) root.classList.add('head-ready');
    } catch (error) {
  ...
  function renderFigure(box, visual = draft()) {
    if (!box) return;
    const key = JSON.stringify({ visual, identity: identity() });
    if (box.dataset.ascFigureKey === key) return;
    box.dataset.ascFigureKey = key;
    box.classList.add('asc-figure-frame');
    box.innerHTML = `<div class="asc-figure"><div class="asc-figure-body">${bodySvg(visual)}</div><div class="asc-head-fallback" aria-hidden="true"></div></div>`;
  ...
  function profileStep(key, value) {
    return stepper(key, value, PROFILE[key], 'profile');
  }

  function avatarStep(key, value) {
    const values = valuesFor(key);
    return stepper(key, value, values, 'avatar', (item, index) => cfg().optionLabel(key, item, index));
  }
  ...
  function bindControls(card) {
    ...
    if (name) {
      ...
      name.oninput = () => {
        ...
        document.querySelectorAll('.avatar-name').forEach(node => {
          node.textContent = name.value || 'Unnamed Hero';
        });
  ...
  function patchHome() {
    const title = [...document.querySelectorAll('.section-title')]
      .find(node => node.textContent.trim() === 'Character progress');
  ...
  function patchCombat() {
    ...
    if (result) {
      ...
      if (!box) {
        box = document.createElement('div');
        box.className = 'combat-result-lorelei';
        result.prepend(box);
      }
      renderFigure(box, draft());
    }
  ...
  function patchAllFigures() {
    document.querySelectorAll('.avatar-orb:not(.asc-preview-figure)')
      .forEach(box => renderFigure(box, draft()));
    patchHome();
    patchCombat();
  }
